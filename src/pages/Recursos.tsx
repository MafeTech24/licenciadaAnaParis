      ...
      >
        ...
        >
          ...
          <div className="p-6 sm:p-8">
            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider mb-3 ${badgeClass}`}>
              {product.badge}
            </span>

            <h2 className="font-display font-semibold text-2xl text-brand-bg-dark leading-snug mb-1">
              {product.title}
            </h2>
            <p className="text-xs font-body font-medium text-brand-accent mb-4">
              {product.subtitle}
            </p>
            <p className="text-sm font-body font-light text-brand-text-muted leading-relaxed mb-5">
              {product.description}
            </p>
            ...
            {product.extras && (
              <div className="mb-6">
                <button
                  ...
                  className="flex items-center justify-between w-full py-2.5 text-xs font-semibold text-brand-sage-dark hover:text-brand-bg-dark transition-colors border-t border-b border-brand-sage/15 focus:outline-none cursor-pointer"
                >
                  <span>{extrasOpen ? `Ocultar ${product.extras.label.toLowerCase()}` : `Ver ${product.extras.label.toLowerCase()}`}</span>
                  {extrasOpen ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                </button>
                <AnimatePresence>
                  {extrasOpen && (
                    <motion.ul
                      ...
                      className="overflow-hidden py-3 space-y-2"
                    >
                      {product.extras.items.map((item, idx) => (
                        <li key={idx} className="flex gap-2 items-start text-[11px] font-body font-light text-brand-text/90 leading-snug">
            ...
            <p className="text-center text-[10px] text-brand-text-muted/70 mt-2">
    ...
    <div className="pt-28 pb-24 bg-brand-bg min-h-screen text-brand-text select-none relative">
      ...
      <div className="w-full max-w-[1000px] mx-auto px-6 relative z-10">
          ...
          {PACKS.map((pack, packIdx) => (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col flex-1">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-accent/15 text-brand-accent border border-brand-accent/30 text-[9px] font-semibold uppercase tracking-widest mb-4 self-start">
                  <Sparkles className="w-3 h-3" />
                  {pack.badge}
                </span>

                <h2 className="font-display font-semibold italic text-2xl leading-tight text-brand-cream mb-2">
                  {pack.title}
                </h2>
                <p className="font-body font-light text-brand-cream/70 text-sm leading-relaxed mb-6">
                  {pack.subtitle}
                </p>

                {/* Ítems del pack */}
                <div className="space-y-2.5 mb-7 flex-1">
                  <p className="text-[9px] font-semibold uppercase tracking-widest text-brand-accent/70 mb-3">
                    Incluye
                  </p>
                  {pack.items.map((productId) => {
                    ...
                    return (
                      <div
                        key={productId}
                        className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/8 rounded-[10px] group"
                      >
                        <span className="text-base">{prod.badge.split(' ')[0]}</span>
                        <span className="flex-1 text-[11px] font-body text-brand-cream/80 leading-snug">
                          {prod.title}
                        </span>
                        <button
                          onClick={() => openModal(productId, pack.hotmartUrl, `Adquirir ${pack.title}`)}
                          className="text-[9px] font-semibold text-brand-accent/60 hover:text-brand-accent underline underline-offset-2 transition-colors cursor-pointer shrink-0"
                        >
                          Ver detalle →
                        </button>
                      </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full inline-flex items-center justify-center gap-2 py-3.5 rounded-full bg-brand-accent text-white text-[10px] font-semibold uppercase tracking-widest hover:bg-[#A95A2F] hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 shadow-md cursor-pointer"
                >
                  <ShoppingBag className="w-3.5 h-3.5" />
                  Adquirir Pack
        ...
        >
          ...
          <span className="font-body text-[10px] font-semibold uppercase tracking-widest text-brand-text-muted">
            ...
            return (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider mb-2 self-start ${badgeClass}`}>
                    {product.badge}
                  </span>
                  <p className="font-display font-semibold text-sm text-brand-bg-dark leading-snug mb-1 line-clamp-2">
                    {product.title}
                  </p>
                  <p className="text-[10px] font-body font-light text-brand-text-muted leading-relaxed mb-4 flex-1 line-clamp-3">
                    {product.description}
                  </p>

                  <button
                    onClick={() => openModal(product.id, pack.hotmartUrl, `Adquirir ${pack.title}`)}
                    className="w-full py-2 rounded-full border border-brand-sage/40 text-brand-sage-dark text-[9px] font-semibold uppercase tracking-wider hover:bg-brand-bg-sage hover:border-brand-sage-dark transition-all duration-200 cursor-pointer"
                  >
